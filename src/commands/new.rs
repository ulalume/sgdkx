...
pub fn create_makefile(project_path: &Path, sgdk_path: &Path) {
    ...
    let makefile_content = if cfg!(target_os = "windows") {
        // Windows path format - use actual path as %HOME% isn't standard in Windows Makefiles
        format!(
...
"#,
        ...
        // macOS path format with $(HOME) substitution
        r#"# SGDK Makefile - Generated by sgdktool
# Note: This file is in .gitignore to avoid committing personal paths
# If needed, adjust the GDK path to match your SGDK installation

GDK = $(HOME)/Library/Application\ Support/sgdktool/SGDK
include $(GDK)/makefile_wine.gen
"#
        .to_string()
    } else {
        // Linux path format with $(HOME) substitution
        r#"# SGDK Makefile - Generated by sgdktool
# Note: This file is in .gitignore to avoid committing personal paths
# If needed, adjust the GDK path to match your SGDK installation

GDK = $(HOME)/.config/sgdktool/SGDK
include $(GDK)/makefile_wine.gen
"#
        .to_string()
    };
